...
#################################################################################*/
function img_resize($src, $dest, $width, $height,  $quality=100, $rgb=0xFFFFFF, $fon=0) //для миниатюр
{
	...
	$x_ratio = $width / $size[0];  //на сколько уменьшаем width (0.1  0.2)
	$y_ratio = $height / $size[1];

	//$ratio       = min($x_ratio, $y_ratio); //ст
	
	$ratio       = max($x_ratio, $y_ratio);	//но  выбираем макс
	
	
	$use_x_ratio = ($x_ratio == $ratio); //использовать ли х уменьшение 

	$new_width   = $use_x_ratio  ? $width  : floor($size[0] * $ratio); //уменьшаем
	$new_height  = !$use_x_ratio ? $height : floor($size[1] * $ratio);
	
	
	//$new_left    = $use_x_ratio  ? 0 : floor(($width - $new_width) / 2); //Точка на изображении назначения, которая определяет 
	//левый край прямоугольника в который будет вставляться копируемая область.
	//$new_top     = !$use_x_ratio ? 0 : floor(($height - $new_height) / 2);//Точка на изображении назначения, которая определяет 
	//верхний край прямоугольника в который будет вставляться копируемая область.
	...
	else
	{
		...
		//$idest = imagecreatetruecolor($new_width, $new_height); //стар
	}
...
*/
	...
	imagejpeg($idest, $dest, $quality); //запись нового файла
...
function img_resizeFoto($src, $dest, $width, $height,  $quality=100, $rgb=0xFFFFFF, $fon=0) //для всех фоток
{
	if (!file_exists($src)) return false;
